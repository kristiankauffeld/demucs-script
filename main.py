...
def output_results(
    original_source: torch.Tensor, predicted_source: torch.Tensor, source: str
):
    print(
        "SDR score is:",
        separation.bss_eval_sources(
            original_source.detach().numpy(), predicted_source.detach().numpy()
        )[0].mean(),
    )
...
save_path = "/Users/kristian/Documents/dev/demucs/"
for source_name, source_audio in audios.items():
    save_audio(source_audio.cpu(), save_path + source_name + ".wav")
